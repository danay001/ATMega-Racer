...
void nokia_lcd_drawImage( uint8_t* image, uint8_t x, uint8_t y )
{
		...
		for ( byteColumn = 0; byteColumn < lineSize; byteColumn++ )
		{
			for ( bit = 7; bit >= 0; bit--)
			{
				if ( (image[row*lineSize + byteColumn + 2] & (1 << bit) ) ){
					nokia_lcd_set_pixel( x + (byteColumn * 8) + ( 7 - bit ), y + height - row, BLACK );
				}
				else{
					nokia_lcd_set_pixel( x + (byteColumn * 8) + ( 7 - bit ), y + height - row, 0 );
				}
			}
		}
	}
